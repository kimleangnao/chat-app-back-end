...
const createInvite = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json({msg: 'success', id: invite})
}
const joinRequest = async (req, res) => {
    ...
    const {groupId, _id: inviteId} = req.body

    const join = await InviteLink.findOne({groupId, _id: inviteId})
    if(!join){
        throw new NotFoundError(`No invite link found for ${inviteId}`)
    }
